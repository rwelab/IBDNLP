...
def Multi_Word_NER(doc):
    ...
    diarrhea_bag_of_words = [
        ...
        "(unform[a-zA-Z]*[ ]{1,}(stool[a-zA-Z]*|bm[a-zA-Z]*|bowel[ ]{1,}movement[a-zA-Z]*))",
        "(constipa[a-zA-Z]*)",
    ]
        ...
        for regex in ent_category:
            for match in re.finditer(regex, doc.text, re.IGNORECASE):
                ...
                if Label == "Diarrhea_gen":
                    ...
                    pos_result = check_word(
                        positive_indicators,
                        [token.text for token in doc if not len(token.ent_type_) != 0],
                        span.start,
                    )
                    # print(pos_result)
                    neg_result = check_word(
                        adj_negation,
                        [token.text for token in doc if not len(token.ent_type_) != 0],
                        span.start,
                    )
                    ...
                    pos_result = check_word(
                        positive_indicators, [token.text for token in doc], span.start
                    )
            ...
            for match in trial:
                ...
                if span not in spans:
                    ...
                    if result == True:
                        ...
                        spans.append(span)
                        spans_gen.append(
                            (match.group(0), match.group(1), match.spans(1))
                        )
        except:
            pass
...
def present_tense(nlp_spacy, sentence, ent_label, ent_start):
    ...
    for token in doc:
        ...
        if (
            token.ent_type_ in ent_label
            or (
                ent_label == "Diarrhea"
                and (
                    token.ent_type_ == "Diarrhea_neg"
                    or token.ent_type_ == "Diarrhea_imp"
                )
            )
            and token.i == ent_start
        ):
...
def remove_ROS(note):
    try:
        if re.search(r"HPI\/ROS", note) is not None:
            return re.search(r"(.*?)HPI\/ROS(.*)\: (Yes|No)(.*?)", note).group(
                1
            ) + re.search(r"(.*?)HPI\/ROS(.*)\: (Yes|No)(.*?)", note).group(4)
