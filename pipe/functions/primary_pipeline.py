...
def get_fecal_answer1(note):
    try:
        return re.search(r'.*? blood in your stool\:\s(.*?)\s', note, re.IGNORECASE).group(1)
    except:
        return None
...
def get_fecal_answer(note):
    try:
        answer1 =  re.search(r'.*? blood in your stool\:\s(.*?)\s', note, re.IGNORECASE).group(1)
    except:
        try:
            answer2 =  re.search(r'.*? or sticky bowel movements:\s(.*?)\s', note, re.IGNORECASE).group(1)
            return answer2
        except:
            return None
...
def extract_HPI (note): 
    ...
    if re.search(r'(History\s*of\s*Present\s*Illness\s*|\s*HPI)', note, re.IGNORECASE) is not None:
        try:
            r = re.search(r'(?:History\s*of\s*Present\s*Illness|HPI)(.*?)(Medical\s*(:?H|h)istory|Surgical\s*(:?H|h)istory)', note)     
            return [r.group(1), r.start(1)]
        except:
            # checks for the presence of a PMS or PSH Header
            if re.search(r'((Past)?Medical(\/Surgical)?|Surgical)\s*(:?H|h)istory', note) is not None:
                try:
                    r = re.search(r'(.*?)(:?(Past)?Medical(\/Surgical)?|Surgical)\s*(:?H|h)istory', note)
                    return [r.group(1), r.start(1)]
                except: #checks for the presence of a Physical exam or review of systems Header
                    if re.search(r'Physical\s*Exam|Review\s*of\s*Systems', note) is not None:
                        try:
                            r = re.search(r'(.*?)(Physical\s*Exam\s*|\s*Review\s*of\s*Systems)', note)
                            return [r.group(1), r.start(1)]
                        except:
                            return [1,1]
            else:
                if re.search(r'Physical\s*Exam|Review\s*of\s*Systems', note, re.IGNORECASE) is not None:
                        try:
                            r = re.search(r'(.*?)(Physical\s*Exam\s*|\s*Review\s*of\s*Systems)', note)
                            return [r.group(1), r.start(1)]
                        except:
    ...
    elif re.search(r'(:?(Past)?Medical(/Surgical)?|Surgical)\s*(:?H|h)istory', note) is not None:
    ...
    elif re.search(r'(Physical\s*Exam|Review\s*of\s*Systems)', note, re.IGNORECASE) is not None:
        try:
...
def Explore_IBD_Events(note):
    try:
        word = re.search(r'\s(\w+)\s*?ibd\s*(event:|history:)', note, re.IGNORECASE).group(1)
        return word.lower()
    except:
        return 1
...
def find_IntervalHistory(HPI, HPI_offset):
    if isinstance(HPI, str):
        ...
        try: 
            r = re.search(r'(?: interval[ ]{1,}(?:History|Event|hx)|Interim(?: History| Event| Hx)?|20\d\d:)(.*?)\Z', HPI, re.IGNORECASE)
            print(r)
...
def find_IntervalHistory2(text, HPI_offset):
    if isinstance(text, str):
        ...
        while True:
            try:
                r = re.search(r'(?:Interval\s*(?:History|Event|hx)|Interim(?: History|Event|hx)?|20\d\d:)(.*?)\Z', text, re.IGNORECASE)
