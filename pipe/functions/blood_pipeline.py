...
row_names = ['Pain_ROS', 'pain_mention_Interval_History', 'pain_mention_Previous_note', 'pain_mention_HPI']
row_names_blood = ['Fecal_Blood_Value', 'blood_mention_Interval_History', 'blood_mention_Previous_note', 'blood_mention_HPI']
rows_names_cr = ['CR_ROS', 'cr_mention_Interval_History', 'cr_mention_Previous_note', 'cr_mention_HPI']
row_names_well = ['Well_ROS', 'well_mention_Interval_History', 'well_mention_Previous_note', 'well_mention_HPI']
...
def find_blood(note):
    ...
    bag_of_words = ['blood', 'bleed', 'hematochezia', 'BRB[a-zA-Z]*?', 'maroon', 'melena','bright[ ]{1,red[ ]{1,}blood', 'red(?:.*)stool']
    matches = {}
    keywords = []
    ...
    for index, match in enumerate(ordered_matches):
        #print(match)
        if re.search(r' test ', match, re.IGNORECASE) is not None or re.search(r'blood\s*work', match, re.IGNORECASE) is not None or re.search(r'blood[ ]{0,}sugar', match, re.IGNORECASE) is not None or re.search(r' clot ', match, re.IGNORECASE) is not None or re.search(r'transfusion', match, re.IGNORECASE) is not None or re.search(r'abscess', match, re.IGNORECASE) is not None or re.search(r'blood\s*count', match, re.IGNORECASE) is not None or re.search(r'blood\s*culture', match, re.IGNORECASE) is not None or re.search(r'blood\s*loss', match, re.IGNORECASE) is not None: #or re.search(r' stoma ', match, re.IGNORECASE) is not None #or re.search(r'anastomotic', match, re.IGNORECASE) is not None: #or re.search(r'output', match, re.IGNORECASE) is not None or re.search(r'fistula', match, re.IGNORECASE) is not None:
...
def find_pain(note, nlp_spacy):
    bag_of_words = ['abd[a-zA-Z]*[ ]{1,}(pain|cramp[a-zA-Z]*|discomfort|ache)', 'rlq[ ]{1,}(pain|cramp[a-zA-Z]*|discomfort|ache)',
                'epigastric[ ]{1,}(pain|cramp[a-zA-Z]*|discomfort|ache)' , 'llq[ ]{1,}(pain|cramp[a-zA-Z]*|discomfort|ache)', 
                    'pelvic[ ]{1,}(pain|cramp[a-zA-Z]*|discomfort|ache)', 
                'stomach[ ]{1,}(pain|cramp[a-zA-Z]*|discomfort|ache)',
                    'luq[ ]{1,}(pain|cramp[a-zA-Z]*|discomfort|ache)', 'ruq[ ]{1,}(pain|cramp[a-zA-Z]*|discomfort|ache)', 
...
def remove_ROS(note):
    try:
        if re.search(r'HPI\/ROS', note) is not None:
            return re.search(r'(.*?)Answers for HPI\/ROS(.*)\: (Yes|No)(.*?)', note).group(1) + re.search(r'(.*?)Answers for HPI\/ROS(.*)\: (Yes|No)(.*?)', note).group(4)
...
#     processed_note = re.sub(r'[a-zA-Z]/[a-zA-Z]', ', ', processed_note) #removes / and replaces with comma
...
def return_abdominal_spacy():
    ...
    ruler.add_patterns([{"label": 'BLOOD', "pattern": [{"TEXT": {"REGEX": "(b|B)lood[a-zA-Z]*" }}]},
                    {"label": 'BLOOD', "pattern": [{"TEXT": {"REGEX": "(b|B)leed[a-zA-Z]*" }}]},
                    {"label": 'BLOOD', "pattern": [{"TEXT": {"REGEX": "(h|H)ematochezia[a-zA-Z]*"}}]},
                    {"label": 'BLOOD', "pattern": [{"TEXT": {"REGEX": "(BRBPR[a-zA-Z]*|brbpr[a-zA-Z]*)"}}]},
                    ...
                    {"label": 'PAIN_GEN', "pattern": [{"TEXT": {"REGEX": "(c|C)ramp[a-zA-Z]*"}}]},
                    #{"label": 'PAIN', "pattern": [{"TEXT": {"REGEX": r"(a|A)bdominal"}}, {"TEXT": {"REGEX": r"(p|P)ain"}}]},
                    {"label": 'PAIN', "pattern": [{"TEXT": {"REGEX": r"(a|A)bd[a-zA-Z]*"}}, {"TEXT": {"REGEX": r"(p|P)ain"}}]},
                    {"label": 'PAIN', "pattern": [{"TEXT": {"REGEX": r"(R|r)lq"}},  {"TEXT": {"REGEX": r"(p|P)ain"}}]},
                    {"label": 'PAIN', "pattern": [{"TEXT": {"REGEX": r"(R|r)uq"}},  {"TEXT": {"REGEX": r"(p|P)ain"}}]},
                    {"label": 'PAIN', "pattern": [{"TEXT": {"REGEX": r"llq"}},  {"TEXT": {"REGEX": r"(p|P)ain"}}]},
                    {"label": 'PAIN', "pattern": [{"TEXT": {"REGEX": r"LLQ"}}, {"TEXT": {"REGEX": r"(p|P)ain"}}]},
                    {"label": 'PAIN', "pattern": [{"TEXT": {"REGEX": r"(P|p)elvic"}},  {"TEXT": {"REGEX": r"(p|P)ain"}}]},
                    {"label": 'PAIN', "pattern": [{"TEXT": {"REGEX": r"(A|a)bdominal"}}, {"TEXT": {"REGEX": r"(D|d)iscomfort"}}]},
                    {"label": 'PAIN', "pattern": [{"TEXT": {"REGEX": r"(O|o)bstruct[a-zA-Z]*"}}, {"TEXT": {"REGEX": r"(S|s)ymptom[a-zA-Z]*"}}]},
                    ...
                    {"label": 'PAIN_GEN', "pattern": [{"TEXT": {"REGEX": "(p|P)ain[a-zA-Z]*"}}]},
                    ...
                    {"label": 'CURRENT', "pattern": [{"TEXT": {"REGEX": '(l|L)ast'}}, {"TEXT": {"REGEX": '(w|W)eek'}}]},
                    ##{"label": 'CURRENT', "pattern": [{"TEXT": {"REGEX": "(n|N)ow"}}]},
                    {"label": 'FORMED', "pattern": [{"TEXT": {"REGEX": "(F|f)ormed"}}]},
                    {"label": 'Clin_Rem', "pattern": [{"TEXT": {"REGEX": 'clin[a-zA-Z]*'}}, {"TEXT": {"REGEX": 'rem[a-zA-Z]*'}}]}, 
                    {"label": 'Clin_Rem', "pattern": [{"TEXT": {"REGEX": 'remission'}}]},
                    {"label": 'Clin_Rem', "pattern": [{"TEXT": {"REGEX": 'asymptomatic[a-zA-Z]*'}}]},
                    {"label": 'Clin_Rem', "pattern": [{"TEXT": {"REGEX": '(n|N)o'}}, {"TEXT": {"REGEX": 'sx[a-zA-Z]*'}}]}, 
...
### additional term sets can be added to negspacy-- for full documentation see: https://pypi.org/project/negspacy/
    ...
    ts.add_patterns({
            "preceding_negations": ["hasn't noticed", 'non', 'non-', "no", "none", "no further", "deny", "hasnt had any", "hasn't had", "hasn't",'hasnt', 'resolution', 'resolution of'],
            "following_negations": ["resolved", "subsided", "none now"],
            "pseudo_negations": ["does not typically", 'almost resolved', "no significant"],
            "termination": ['endorses', 'and only', 'scant', 'intermittent', 'with', 'then', ';', 'severe', 'infrequent']
...
def Multi_Word_NER(doc):
    ...
    cr_bag_of_words = ['(clin[a-zA-Z]*[ ]{1,}rem[a-zA-Z]*)',
                       '(quiescent)', 
    ...
    pain_bag_of_words = ['(abd[a-zA-Z]*[ ]{1,}(pain|cramp[a-zA-Z]*|discomfort|ache))', '(rlq[ ]{1,}(pain|cramp[a-zA-Z]*|discomfort|ache))',
                '(epigastric[ ]{1,}(pain|cramp[a-zA-Z]*|discomfort|ache))' , '(llq[ ]{1,}(pain|cramp[a-zA-Z]*|discomfort|ache))', 
                    '(pelvic[ ]{1,}(pain|cramp[a-zA-Z]*|discomfort|ache))', 
                '(stomach[ ]{1,}(pain|cramp[a-zA-Z]*|discomfort|ache))',
                    '(luq[ ]{1,}(pain|cramp[a-zA-Z]*|discomfort|ache))', '(ruq[ ]{1,}(pain|cramp[a-zA-Z]*|discomfort|ache))'
    ...
    current_bag_of_words = ['(past[ ]{1,}week)', '(last[ ]{1,}week)', '(this[ ]{1,}week)']
    
    
   
    
    for ent_category, Label in zip([blood_bag_of_words, pain_bag_of_words, 
                                    bag_of_words_gen, Well_bag_of_words, 
                                    Well_bag_of_words_neg, cr_bag_of_words, 
                                    current_bag_of_words], ['BlOOD', 'PAIN', 'PAIN_gen', 'WELL','WELL_neg' ,'Clin_Rem', 'CURRENT']):
...
def get_pain_adjective(sentence, doc, location, vectors_model = sci_spacy_model):
    ...
    positive_indicators = ['epigastric', 'flank', 'abdominal', 'abdomen', 'stomach', 'abd', 'pelvic', 'pelvis', 'periumbilical', 'quadrant', 'RLQ', 'rlq', 'LLQ', 'llq', 'RUQ', 'ruq', 'luq', 'LUQ']
    anatomy_terms = ['joint', 'eye', 'abdominal', 'wrist', 'back', 'neck', 'ankle', 'rectal', 'msk', 'medication']
            ...
            if len(set(children_left_text).intersection(positive_indicators)) == 0:
                ...
                #print(True)
                    for word in children_left_text:
                        #print(word)
                        if word_vector_comparison(anatomy_terms, .30, word):
                            return False
    ...
    for index, mention in enumerate(mentions):
        ...
        for index, ent in enumerate(doc.ents):
            ...
            if ent.label_ in Label_1:
                ...
                present_tense_true, Tense = present_tense(nlp_spacy, doc.text, ent.label_, ent.start)
...
def encode_column(df1, df2, column_to_encode, encoded_column_name, df_same):
    ...
    for index, values in zip(values_index, values_list):
        ...
        if 'CURRENT' in Tenses:
            ...
            True_Blood_Values = [value for index, value in enumerate(Blood_Values) if index in True_index]
            ...
            True_index = [index for index, tense in enumerate(Tenses) if tense == 'NO_VERB']
            True_Blood_Values = [value for index, value in enumerate(Blood_Values) if index in True_index]
            ...
            True_index = [index for index, tense in enumerate(Tenses) if tense == 'SIMPLE_PRESENT']
            True_Blood_Values = [value for index, value in enumerate(Blood_Values) if index in True_index]
...
def binary_accuracy(y_predict, y_value, indices):
    ...
    for prediction, value, index in zip(y_predict, y_value, indices):
        if math.isnan(prediction) and math.isnan(value):
            positive += 1 
        elif not math.isnan(prediction) and math.isnan(value):
            false_negative += 1
            false_negative_index.append((index, prediction, value))
        elif math.isnan(prediction) and not math.isnan(value):
            false_positive += 1 
            false_positive_index.append((index, prediction, value))
            
        
        elif not math.isnan(prediction) and not math.isnan(value):
            positive += 1
    ...
    accuracy = positive / (len(y_predict) + 1) 
